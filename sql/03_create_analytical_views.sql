...
-- ============================================================================
-- PRODUCT TYPE NORMALIZATION (Run before creating views)
-- ============================================================================
-- Normalize product type variations to ensure consistent categorization
-- This fixes variations like "Cosmetic" vs "Cosmetics", "stationary" vs "stationery", etc.

UPDATE dim_product
SET product_type = CASE
    -- Cosmetics variations
    WHEN LOWER(TRIM(product_type)) IN ('cosmetic', 'cosmetics') THEN 'Cosmetics'
    
    -- Electronics variations
    WHEN LOWER(TRIM(product_type)) IN ('electronic', 'electronics', 'electronics and tech', 'electronics and technology') 
        THEN 'Electronics And Technology'
    
    -- Toys variations
    WHEN LOWER(TRIM(product_type)) IN ('toy', 'toys', 'toys and entertainment', 'entertainment') 
        THEN 'Toys And Entertainment'
    
    -- Apparel variations
    WHEN LOWER(TRIM(product_type)) IN ('apparel', 'clothing') THEN 'Apparel'
    
    -- Grocery variations
    WHEN LOWER(TRIM(product_type)) IN ('grocery', 'groceries') THEN 'Grocery'
    
    -- Readymade Dinner variations
    WHEN LOWER(TRIM(product_type)) IN ('readymade dinner', 'readymade dinners') THEN 'Readymade Dinner'
    
    -- Kitchenware variations
    WHEN LOWER(TRIM(product_type)) IN ('kitchenware', 'kitchen ware') THEN 'Kitchenware'
    
    -- Tools variations
    WHEN LOWER(TRIM(product_type)) IN ('tool', 'tools') THEN 'Tools'
    
    -- Accessories variations
    WHEN LOWER(TRIM(product_type)) IN ('accessory', 'accessories') THEN 'Accessories'
    
    -- Stationary/Stationery variations
    WHEN LOWER(TRIM(product_type)) IN ('stationary', 'stationery', 'stationary and school supplies', 
                                       'stationery and school supplies', 'school supplies') 
        THEN 'Stationary And School Supplies'
    
    -- Furniture (no variations, but ensure consistency)
    WHEN LOWER(TRIM(product_type)) = 'furniture' THEN 'Furniture'
    
    -- Keep existing value if no normalization needed
    ELSE product_type
END
