"""
ShopZada Marketing Department - Parquet Export DAG.

This DAG exports all Marketing Department staging tables to Parquet format.
"""
...
def export_department_to_parquet(**_context) -> None:
    """Export Marketing Department staging tables to Parquet."""
...
with DAG(
    dag_id="shopzada_marketing_dept_parquet",
    default_args=default_args,
    start_date=datetime(2025, 1, 1),
    schedule_interval=None,  # Manual trigger only
    catchup=False,
    max_active_runs=1,
    tags=["shopzada", "kimball", "staging", "marketing", "parquet"],
    description=f"Export {DEPARTMENT} staging tables to Parquet format.",
) as dag:
    export_to_parquet = PythonOperator(
        task_id="export_marketing_dept_to_parquet",
