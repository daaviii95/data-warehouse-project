"""
ShopZada Business Department - Parquet Export DAG.

This DAG exports all Business Department staging tables to Parquet format.
"""
...
def export_department_to_parquet(**_context) -> None:
    """Export Business Department staging tables to Parquet."""
...
with DAG(
    dag_id="shopzada_business_dept_parquet",
    default_args=default_args,
    start_date=datetime(2025, 1, 1),
    schedule_interval=None,  # Manual trigger only
    catchup=False,
    max_active_runs=1,
    tags=["shopzada", "kimball", "staging", "business", "parquet"],
    description=f"Export {DEPARTMENT} staging tables to Parquet format.",
) as dag:
    export_to_parquet = PythonOperator(
        task_id="export_business_dept_to_parquet",
