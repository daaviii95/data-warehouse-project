...
### 4. `order_with_merchant_data_scenario4.csv`

**Purpose:** Required for fact_orders loading (provides merchant_id and staff_id)

**Valid Records (16 rows):**
- All rows with valid order_ids matching order_data_scenario4.csv
- All have valid merchant_id and staff_id

**Note:** This file should match the order_ids in order_data_scenario4.csv. Some orders will be rejected due to invalid data in order_data, but this file provides the merchant/staff mapping for valid orders.

**Expected Results:**
- Valid orders will merge successfully with order_data
- Invalid orders will be rejected during fact_orders loading

### 5. `transactional_campaign_data_scenario4.csv`
...
1. **Copy test files to appropriate directories:**
   ...
   # Enterprise data (required for fact_orders)
   cp data/Test/order_with_merchant_data_scenario4.csv "data/Enterprise Department/"
   ```
